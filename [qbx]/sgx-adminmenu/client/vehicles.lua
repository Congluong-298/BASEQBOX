...
-- Set Fuel (Command)
RegisterNetEvent('sgx-adminmenu:client:SetFuel', function(amount, adminSource)
    if cache.vehicle then
        if Config.Fuel == "ox_fuel" then
            Entity(cache.vehicle).state.fuel = tonumber(amount)
        else
            exports[Config.Fuel]:SetFuel(cache.vehicle, tonumber(amount))
        end
        
        local playerName = QBCore.Functions.GetPlayerData().charinfo.firstname .. ' ' .. QBCore.Functions.GetPlayerData().charinfo.lastname
        QBCore.Functions.Notify(locale("fuel_set", amount), 'success')
        
        -- Notify admin who executed the command
        if adminSource ~= cache.ped then
            TriggerServerEvent('sgx-adminmenu:server:NotifyAdmin', adminSource, locale("fuel_set", amount) .. ' for ' .. playerName)
        end
    else
        QBCore.Functions.Notify(locale("not_in_vehicle"), 'error')
        
        -- Notify admin that target is not in vehicle
        if adminSource ~= cache.ped then
            TriggerServerEvent('sgx-adminmenu:server:NotifyAdmin', adminSource, locale("fuel_target_not_in_vehicle"))
        end
    end
end)
...
RegisterNetEvent('sgx-adminmenu:client:ChangePlate', function(data, selectedData)
    ...
    if cache.vehicle then
        ...
        Wait(100)
        exports['qs-vehiclekeys']:GiveKeys(QBCore.Functions.GetPlate(cache.vehicle), cache.vehicle, true)
    else
        QBCore.Functions.Notify(locale("not_in_vehicle"), 'error')
    end
end)
