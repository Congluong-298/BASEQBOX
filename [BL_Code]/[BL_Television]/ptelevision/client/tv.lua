...
function BroadcastMenu() 
    ...
    local input = lib.inputDialog('Phát sóng trực tiếp', {'Tên kênh:', 'URL phát sóng:'})
...
function WebBrowserMenu()
    lib.hideMenu()
    local input = lib.inputDialog('Trình duyệt web', {'URL:'})
...
function VideoMenu()
    lib.hideMenu()
    local input = lib.inputDialog('Trình phát video', {'URL:'})
    if input then 
        TriggerServerEvent("ptelevision:event", CURRENT_SCREEN, "ptv_status", {
            type = "play",
            url = input[1]
...
function VolumeMenu()
    lib.hideMenu()
    local input = lib.inputDialog('Âm lượng', {'Đặt âm lượng (0-20):'})
    if (tonumber(input[1])) then 
        local coords = CURRENT_SCREEN.coords
        SetVolume(coords, tonumber(input[1])/20)
    end
    Citizen.Wait(300) 
    OpenTVMenu()
end
...
function OpenTVMenu() 
    ...
    lib.registerMenu({
        ...
        options = {
            {label = 'Videos', description = 'Phát video hoặc phát trực tuyến trên màn hình.'},
            {label = 'Trình duyệt web', description = 'Truy cập web qua TV của bạn.'},
            {label = 'Kênh truyền hình', values = ChannelList.display, description = 'Các kênh truyền hình trực tiếp!', defaultIndex = ChannelList.current},
            {label = 'Tương tác với màn hình', description = 'Cho phép bạn kiểm soát các yếu tố trên màn hình.'},
            {label = 'Đặt âm lượng', description = 'Đặt âm lượng TV của bạn (Dành cho chính bạn).'},
            {label = 'Đóng MENU', close = true},
        }
