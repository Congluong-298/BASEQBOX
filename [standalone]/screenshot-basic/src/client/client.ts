...
exp('requestScreenshot', (options: any, cb: (result: string) => void) => {
    ...
    if (realOptions.width) realOptions.width = parseInt(realOptions.width);
    if (realOptions.height) realOptions.height = parseInt(realOptions.height);
...
exp('requestScreenshotConvert', (options: any, cb: (result: string) => void) => {
