...
`
    ...
    handleRequest(request: ScreenshotRequest) {
        ...
        if (request.width && request.height) {
            ...
            resizeCanvas.width = request.width;
            resizeCanvas.height = request.height;
            const resizeCtx = resizeCanvas.getContext('2d');
            
            // Draw original canvas into resized canvas
            resizeCtx.drawImage(canvas, 0, 0, window.innerWidth, window.innerHeight, 0, 0, request.width, request.height);
        ...
        const imageURL = finalCanvas.toDataURL(type, request.quality);
