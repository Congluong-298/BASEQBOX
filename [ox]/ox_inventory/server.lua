		...
		for _, v in pairs(data) do
			if type(v) == 'number' or not v.count or not v.slot then
				if server.convertInventory then
					...
					return error(('Kho đồ của người chơi.%s (%s) chứa dữ liệu không hợp lệ. Đảm bảo bạn đã chuyển đổi kho đồ sang đúng định dạng.'):format(player.source, GetPlayerName(player.source)))
				end
			else
